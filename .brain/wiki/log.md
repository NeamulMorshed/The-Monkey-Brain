...
## [2026-09-16] lint | post-restart check of the whole brain
Mechanical layer clean: lint (35 pages, no broken links, no orphans, frontmatter complete, index in sync), doctor 17 ok / 2 warn, repo and installed-cache selftests ALL GREEN (581), both manifests `--strict`. Wiring: 11 hooks registered across 10 events, 27 scripts, 25 skills, 2 agents, the `brain-search` MCP, 5 capability plugins enabled. Live probe of every hook path from the installed cache: session-start injection 2,287 B (554 avg tokens, budget 3,000), 10/10 router cases correct, recall ranks real pages, wiki-check clean on a real page, guards block a `raw-sources/` write and a secret, agent-track blocks a model-less dispatch, Stop silent with nothing unmet, `brain_brief` answers over 40 pages. Open: doctor #6 log-gap (this check, now logged) and #20 the missing `GITHUB_PERSONAL_ACCESS_TOKEN`; doctor #3's contradiction flag is the deliberate stale-snapshot note on [[engine-changelog]]. Found: the running session still executes cache 0.29.1 hooks after the restart — investigated separately.

## [2026-09-16] lint | independent end-to-end test of the installed plugin → 0.33.2
A harness written against the installed cache (not the repo selftest) ran 128 probes: scaffolding, all 15 registered hook commands across 10 events, a 38-prompt router matrix (one per rule plus negatives), the gates, the wiki layer, the 25 skills and their scripts, the `brain-search` MCP, the 6 installed plugins, and a full session lifecycle. 121 passed. Of the 7 failures, 5 were harness mistakes — `raw-sources/` is add-only by design (overwrite and edit are blocked), the brainless-repo hint does keep its lifecycle line, my SKILL.md parser assumed LF, `srs.js` takes `due <deck>`, and `stopCheck` has a deliberate 90-second grace — 1 is the curator-side `GITHUB_PERSONAL_ACCESS_TOKEN`, and 1 was real: the product-design rule missed an adjective between the article and the noun ("design the onboarding user flow", "design the onboarding screen", "redesign the checkout ux" fell through silently). Fixed test-first in 0.33.2 (4 red → green, selftest 589), pushed and reinstalled. Stop nudges re-probed past the grace: fires when pages changed after the last log write, once per session, again in a new session, silent when caught up, and log + git arrive in one block.

## [2026-09-16] lint | CI was red for five pushes → 0.33.3
Asked whether CI was checked: it had not been. `selftest` failed on every push from the brain-hardening wrap (`0fc702c`) through `cd12dfb`, on all 6 matrix jobs. First run: the Windows CRLF manual (fixed in 0.33.1). Since then only the step `Example brain lints clean`: the slug-collision check added in 0.32.0 flagged `examples/claude-code-brain`, where `sources/context-window` and `sources/permission-modes` shared basenames with their concepts. Renamed them `-docs`; `sources:` provenance, *Sources* sections and four doc-specific cross-links retargeted (21 pages); strict lint clean. `/brain:wrap` now runs the workflow `run:` steps locally and checks the pushed commit's CI, with red meaning not done (selftest 590, test-first).
