...
1. Reports `warn`-level learned-ban matches in the text a write just added.
2. The Gap-#9 feedback loop: counts, per file, how many **distinct sessions** have revised it
   (`countSession`, state in `sessions/edit-counts.json`); at the threshold (default 3,
...
`fm.confidence`, else derived from the evidence count (the `BY_EVIDENCE` constant: 0→0.25,
1→0.4, 2→0.6, 3→0.75, 4→0.85, 5+→0.9) via `confidence()` — flagging ≥0.8 `"promote?"` and >30
days old `"stale — prune?"` in `status()`. `promote <name>` moves `pending/` → `active/`
(`move()`) — **the curator's call**; `/brain:review` asks before running it. `prune <name>`
moves to `pruned/` (kept for the record, not deleted). `test <file>` shows which active bans
would fire on a given file (`test()`). **`active`** prints each active rule on one line — the
form forked skills (only `build`, see [[model-routing]]) inject via `instincts.js active` since
a fork misses the session-start block.

**Enforcement — `bans.js`**: `loadBans(brain)` combines two sources, both plain data:
`fromInstincts` (active instincts whose frontmatter carries `ban:`) and `fromPacks` (any
capability pack's `bans.json` — e.g. `plugin/skills/product-design/bans.json` — active only
while a `projects/` workstream declares `pack: <name>` and `status: active`). `findMatches(text,
relPath, bans)` returns the first matching line per ban, filtered by an optional `paths` regex.
Enforcement then splits across two hooks:
- **`guards.js` refuses `enforce: block` matches before the write** (guard #6 — see
  [[plan-and-tdd-gates]]).
- **`instinct-track.js` reports `enforce: warn` matches after the write**, as
  `additionalContext` — advisory, fixable in the same turn.
...
bans checked on every write. `doctor` check 12 (`instinct-queue`) warns when `instincts/pending/`
grows past `INSTINCT_QUEUE_MAX`, prompting the curator to promote or drop the backlog.
...
`loadBans()` re-reads `instincts/active/*.md` and every declared pack's
...
- **`bans.js`'s `compile()`:** an *unquoted* `ban: [0-9]` parses in YAML as a one-item list
