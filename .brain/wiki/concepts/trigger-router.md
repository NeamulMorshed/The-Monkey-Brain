...
**Tightened by [[router-and-drift-review]] (still 0.33.0):** the build's own fixes over-reached, and the review pinned each probe. `HANDBACK_RE` matches only a message that *starts* as someone else's (`[Subagent hand-back]`, "Another Claude session sent a message", `<agent-message>`), so a curator's paste containing "The report follows:" routes again. The research noun is judged by the word *before* it ("the / for / any … research"), so "research papers on RAG" is a request again. `QUESTION_RE` also takes auxiliaries (`is / are / does / did / should / would / will / has / any / was / were`), and the ingest, wrap and init rules are question-exempt too. The doctor audit clause fires only when "review / check the brain" ends the clause ("review the brain-hardening spec" goes to review; "check the wiki for broken links" is not a health report) and no longer claims "audit skills" (career's "skill matrix"). The init rule allows up to three plain words ("set up a fresh brain") but no feature noun or preposition. `DUMP_PIVOT` reads any clause after "we decided" that opens with a build verb (". build", "— implement", ", so build"), unless the prompt opens with "dump". After the one init offer, dev prompts still get their research → plan → build line; without a session id no marker is written.
