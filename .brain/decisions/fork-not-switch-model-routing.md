...
## Decision
The instance manual §5 holds one model-routing table (work class → where → model → effort; Haiku, Sonnet, Opus, Fable), cited by every hook and skill that mentions models. A skill may pin `model:` only together with `context: fork`, and only `build` forks (Sonnet). A fork starts a subagent that cannot see the conversation, runs in the background and misses the session-start injections — worth it for long test-first coding, with the active instincts injected through `instincts.js active`, but not for a report or pack whose `!` script output is the same inline (amended in review, [[token-diet-review]]). Every other skill runs on the session model; judgment skills stay at `effort: high`. The curator picks the session model at the start of a session by the day's work; a context-size nudge (`recall.js`) says "don't switch models mid-session" when context is large.
