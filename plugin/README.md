...
| 2 | UserPromptSubmit | `trigger-router` + `recall` | natural phrases → `/brain:*` routing hints (never blocks); **plan before build** — generic development intent ("add a login feature", "fix the crash on upload") routes to `/brain:plan`, listing the open specs so an already-planned feature goes to `/brain:build` instead (questions — why/what/how — stay silent); **first-prompt recall** — the session's first prompt is searched against the brain and the top 3 matching pages are injected (`MONKEY_BRAIN_RECALL=0` off) |
...
| 6 | Stop + SessionEnd | `wrap` | once-per-session unlogged-work stop gate + **decision-distillation nudge** + **uncommitted-`.brain/`-changes nudge** (git status, mirrors doctor check 7); index stat self-heal; **`qmd update` re-index** when opted in |
...
Actions from the detected stack) · `/brain:home` (one dashboard across every registered project
on the machine, with combined token usage — `hooks/scripts/registry.js` + `home.js`) ·
`/brain:lock` (a committed, expiring team lock) · `/brain:learn` (SM-2 spaced repetition) ·
